...
@register.inclusion_tag('bias_calibrations/partials/instrument_filter_site_table.html')
...
@register.inclusion_tag('bias_calibrations/partials/instrument_filters_at_site.html')
...
@register.inclusion_tag('bias_calibrations/partials/bias_calibrations_cadences_list.html')
def bias_calibrations_cadences_list() -> dict:
    bias_calibrations_cadences = (DynamicCadence.objects.filter(cadence_strategy='BiasCalibrationsCadenceStrategy')
    ...
    print(f"bias_calibrations_cadences : {bias_calibrations_cadences}")

    cadences_data = []
    for cadence in bias_calibrations_cadences:
...
@register.inclusion_tag('bias_calibrations/partials/instrument_observations_at_site.html')
...
@register.inclusion_tag('bias_calibrations/partials/bias_calibrations_targets_list.html')
def bias_calibrations_targets_list() -> dict:
    bias_calibrations_targets = Target.objects.filter(targetextra__key='standard_type', targetextra__value__in=['bias'])
    ...
    context = {'targets_data': [{
        'target': bias_calibrations_target,
        'prev_obs': bias_calibrations_target.observationrecord_set.filter(status='COMPLETED').order_by('-scheduled_end').first(),
        'next_obs': bias_calibrations_target.observationrecord_set.filter(status='PENDING').order_by('scheduled_start').first()
    } for bias_calibrations_target in bias_calibrations_targets]}
    return context
